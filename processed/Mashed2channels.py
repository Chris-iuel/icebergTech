...
conv_layers = 30
conv_f = [5]*conv_layers
conv_num_filters = [30]*conv_layers
num_blocks = 3
...
def conv_calc(conv_input,W_conv,b_conv):
    return tf.nn.relu(conv3d(conv_input, W_conv) + b_conv)
...
def conv_wb_create(conv_f,prev_conv_num_filters,conv_num_filters):
    ...
    W_conv = weight_variable([conv_f, conv_f, channels, prev_conv_num_filters, conv_num_filters])
...
def fully_connected(conv1, conv2, angle):
    with tf.name_scope("fc1"):
        ...
        fc_dim2 = int(numpy.ceil(dimentions[2] / int(2 ** num_blocks)))
        W_fc1 = weight_variable([2 * fc_dim0 * fc_dim1 *fc_dim2 * conv_num_filters[-1] + 1, num_hidden_fc1])
        b_fc1 = bias_variable([num_hidden_fc1])
        h_pool2_conv1_flat = tf.reshape(conv1, [-1, fc_dim0 * fc_dim1 * fc_dim2 * conv_num_filters[-1]])
        h_pool2_conv2_flat = tf.reshape(conv2, [-1, fc_dim0 * fc_dim1 * fc_dim2 * conv_num_filters[-1]])

        temp_fc1 = tf.concat([h_pool2_conv1_flat, h_pool2_conv2_flat], channels)
        input_fc1 = tf.concat([temp_fc1, angle], channels)
...
def main():
    ...
    x1 = tf.placeholder(tf.float32, [None, dimentions[0] * dimentions[1] * dimentions[2]], name='x1')
    x2 = tf.placeholder(tf.float32, [None, dimentions[0] * dimentions[1]] * dimentions[2], name='x2')
    ...
    cross_validation = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=pred))

    train_step = tf.train.AdamOptimizer(learning_rate).minimize(cross_validation)
